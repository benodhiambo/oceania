	...
	<div class="col-md-12 ml-0 pl-0 mb-1">
		...
		<div style="clear:both">
			...
			<input class="hide" type="number"
				...
				border-radius:10px;height: 70px !important;
				width: 145px;outline: none;text-align:center;"
				placeholder='0.00' id="custom_amount_inputs_{{$i}}"/>

			<input class="hide" type="number"
				id="custom_amount_inputs_{{$i}}_buffer"/>
			@endfor
...
<div class="col-md-12 m-0 p-0"
	...
	@for($i=1 ; $i<=env('MAX_ROWS'); $i++)
		...
		margin-left: 0px;">
			...
			<div hidden class="prd-overlay" id = "prd-overlay-{{$s}}">
				<div class="row prd-top-row" 
					style="padding: 0 5px 5px 5px !important">
					<div class="col-sm-2 m-0 p-0 text-center">{{$s}}</div>
					<div class="col-sm-5 m-0 p-0 text-left">
						<span id="prd-name-{{$s}}"></span>
					</div>
					<div class="col-sm-5 m-0 p-0 text-right">
						<span id="prd-myr-{{$s}}"></span>
						<span id="prd-myr-{{$s}}-price" hidden></span>
					</div>
				</div>
					...
					<div class="col-sm-5 m-0 p-0" style="display: flex;height: 35px;">
						...
						<div class="row m-0 p-0 w-100">
							<div class="col-sm-12 m-0 p-0">
								<input class="hide cover-input" type="number"
									style="height: 100%;width:105px"
									placeholder='0.00' id="custom_amount_input_{{$s}}"/>

								<input class="hide" type="number"
									id="custom_amount_input_{{$s}}_buffer"/>

								<button class="cover-input-btn cover-myr-disable"
									id="custom_amount_btn_{{$s}}"
									style="margin: 0 !important;
									position:relative;top:-1px;
									font-size:16px; width:63px !important" 
									onclick="process_custom_amt({{$s}})">
									MYR
								</button>
							</div>
							<!--
				...
				<div class="row m-0 p-0" 
					...
					<div class="col-sm-5 m-0 p-0" style="display: flex;height: 35px;">
						<input class="cover-input" type="number"
							style="width:105px"
							placeholder='0.00' id="custom_litre_input_{{$s}}"/>

						<input class="hide" type="number"
							id="custom_litre_input_{{$s}}_buffer"/>
							
						<button class="cover-input-btn cover-myr-disable"
							style="width:63px!important;font-size:16px"
							id="custom_litre_btn_{{$s}}"
							onclick="litre_to_amt({{$s}})" >
							Litre
						</button>
					</div>
				</div>
				<div class="row prd-bottom-row" 
					...
					<div class="col-sm-7 m-0 p-0" style="display: flex;height: 35px;">
						...
						<button class="btn btn-success
							...
							id="button-credit-ac{{$s}}"
							onclick="select_credit_ac({{$s}})"
						...
						<button class="btn btn-success poa-button-cash
							...
							id="button-cash-payment{{$s}}"
							onclick="select_cash({{$s}})"
							style="">
							Cash
						</button>
					...
					<div class="col-sm-5 m-0 p-0 text-left">
						...
						<input class="justify-content-center align-items-center"
							...
							id="input-cash{{$s}}" placeholder="Received"
							disabled>
						<input id="buffer-input-cash{{$s}}" type="hidden" />

						<button 
							class="btn poa-finish-button-disabled
							finish-button-{{$s}}"
							onclick="cover_process_finish({{$s}})"
							style="margin:0;border-radius:5px;
							height:35px !important;float: right;
