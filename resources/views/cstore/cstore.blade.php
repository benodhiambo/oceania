...
<div class="col-md-12 hidden-md-down"
	...
	<div class="col-md-12" style="background-image:
		...
		<div class="col-md-8 align-items-center d-flex">
			<img src="{{ asset('images/opossum_lgtst-11.png') }}" alt=''
			...
			<div style="width:100%"
				...
				<div class="">
					...
					<button class="btn btn-success btn-sq-lg screend-button
						bg-virtualcabinet"
						onclick="window.open('{{route('local_cabinet.cstore_receipt_landing' , ['date'=>date('dMY',strtotime(now()))] )}}','_blank')"
					...
					<button class="btn cstore-bluecrab-button mb-1"
                        ...
                        id="bluecrab_btn"
						onclick="window.open('{{ route("screen.d") }}')">
					...
					<button class="btn opos-cstore-button"
						...
						<img src="{{asset('images/cstore_exit_button.png')}}"
			...
			<div class=""
				...
				@if (!empty($product))
					...
					<td style="width: 10%;tex-talign:center;"
						class="text-white">
						{{number_format($productData['product_quantity'],2)??""}}
					</td>
					<td style="width: 30%;text-align: right"
						class="text-white pr-4">
						{{number_format($productData['product_amount'],2)??""}}
					</td>
					</tr>
				@else
					<tr>
					{{--
						<td style="width: 75%">
							<img src="{{ asset('images/DKOrecast.png') }}"
								alt="" class="single">
							<span class="text-white">Coke Can</span>
						</td>
						<td style="width: 10%;align:left;"
							class="text-white">2
						</td>
						<td style="width: 20%;text-align: center"
							class="text-white">3,999.00
						</td>
					--}}
					</tr>
				@endif
...
var selected_pump = {{request()->selected_pump ?? 'null'}};
var terminal_sst = {{$terminal->tax_percent ?? 6}};
@if (!empty($product))
products['product{{$product->id??""}}'] = {
	name: '{{str_limit($product->name, $limit = 50, $end = '...')??""}}',
	qty: '{{$productData["product_quantity"]??""}}',
	price: '{{number_format($productData["price"])??""}}',
	total_amount: '{{$productData["product_amount"]??""}}',
	product_id: '{{$product->id??""}}',
	product_systemid: '{{$product->systemid??""}}',
	product_thumbnail: '{{$product->thumbnail_1??""}}',
	removed: 'false',
};
@endif
...
function select_wallet() {
	...
	scan_wallet_qrcode(enable_scanner); 
...
function process_finish(){
	...
	$.post("{{route('local_cabinet.receipt.create.cstore')}}", {
			products: products,
...
$("#product_search").on( "keyup", function( event ) {
	...
	url = '{{route('cstore.search', 'SEARCH_STRING')}}'.
