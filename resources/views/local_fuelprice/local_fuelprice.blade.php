...
@section('content')
	...
	<div class="d-flex mt-0" style="width: 100%">
        ...
        <div style="display:flex;justify-content:flex-end"
			...
			<button class="btn btn-success sellerbuttonwide"
				data-toggle="modal" data-target="" onclick="window.open('{{route('product_mapping')}}','_blank')"
				style="float: right; margin: 0 5px 5px 0;border-radius:10px">
				Product<br>Mapping
			</button>
...
<script>
	...
	"autoWidth": false,
	"ajax": {
		url:"{{route('get_industry_oil_gas_fuel_local_price_datatable')}}",
		type: "POST",
		"data": {
		}
	},
	...
	var table_pump_popup_table = $('#pump_popup_table')
		...
		"autoWidth": false,
		"ajax": {
			url:"{{route('fuel_local_price_pump_detail_datatable')}}",
			type: "POST",
		},
		columns: [
			//{data: 'DT_RowIndex', name: 'DT_RowIndex'},
	...
	function openDetailModal(pump_id) {
		$.post("{{route('fuel_local_price_pump_detail_modal')}}", {
			pump_id:pump_id,
		}).done( res =>{
			$('#res').html(res);
	...
	function trigger_push() {
		$.post("{{route('fuel_local_price.push.hardware')}}").done( res =>{
 ...
 */
	...
	function update_field(field, value, id) {
		$.post('{{route('get_industry_oil_gas_fuel_local_price_update')}}', {
			field:field,
			value:value,
			id:id
		}).done( (res) => {
