		...
		<div class="col-sm-2 pl-0">
			<div class="row mb-0" style="float:right;">
				...
				<input type="hidden" name="" id="stockin"
					value="{{route('openitem.openitem_stockin')}}">
...
@section('script')
    ...
    <script>
            ...
            columns: [
                ...
                // {data: 'product.systemid', name: 'systemid'},
                {
                    data: 'product.systemid', name: 'systemid', render: function (data) {
                        return "<a href='/openitem/products/barcode/" + data + "' target='_blank' id='prd_" + data + "' style='text-decoration: none;'>" + data + "</a>"
                    }
                },
                {
                    data: 'barcode', name: 'barcode', sorting: false,
					// // This has to feed the selected barcode!!
                    // data: 'product.systemid', name: 'barcode', render: function (data) {
					// 	// select barcode from productbarcode where
					// 	// selected = true and systemid=product.systemid
				...
				},
                {
                    data: 'product', name: 'product_name', render: function (data) {
                        return data.thumbnail_1 != null ?
                            "<a href='javascript:void(0)' style='text-decoration: none;padding-top: 15px;' onclick='detailProduct(" + data.id + ")'> <img width='25px' height='25px' style='margin: 0px 0px 0px 0px;object-fit:contain' src='/{{\App\Http\Controllers\OpenitemController::$IMG_PRODUCT_LINK}}" + data.systemid + "/thumb/" + data.thumbnail_1 + "' alt=''> " + (data.name == null ? 'Product Name' : data.name) + "</a>"
                ...
                },
                {
                    data: 'cost', name: 'cost', render: function (data) {
                        ...
                        // return "<a href='#' id='cost_" + systemid + "' style='text-decoration: none;' onclick='add_cost_modal(" + JSON.parse(data)["id"] + "," + JSON.parse(data)["cost"] + "," + JSON.parse(data)["product_id"] + ", this.id)'>" + number_format((JSON.parse(data)["cost"] != null ? JSON.parse(data)["cost"] : 0) / 100, 2) + "</a>"
                        return "<a href='openitem/openitem_cost/" + systemid + "' id='cost_" + systemid + "' style='text-decoration: none;' target='_blank' >" + number_format((JSON.parse(data)["cost"] != null ? JSON.parse(data)["cost"] : 0) / 100, 2) + "</a>"
                ...
                },
                {
                    data: 'qty', name: 'qty', render: function (data) {
                        ...
                        return "<a target='_blank' id='qty_" + systemid + "' href='/openitem/prdledger/" + systemid + "' style='text-decoration: none;'>" + JSON.parse(data)["qty"] + "</a>"
                    }
                },
                {
                ...
                },
                {
                    data: 'loyalty', name: 'loyalty', render: function (data) {
                        return number_format(parseFloat(JSON.parse(data)["loyalty"])) ;
                    }
                },
				{
        ...
        function add_cost_modal(data, old_value , pro, product_id) {
          ...
          $("#pro_id").attr("value", pro);
            ...
            $('#add_cost_modal').on('hidden.bs.modal', function (e) {
                ...
                $.ajax({
                    url: "{{ route('openitem.openitem_save_prd_cost') }}",
                    type: "POST",
                    data: {
        ...
        function detailProduct(data) {
            $.ajax({
                method: "post",
                url: "{{route('openitem.detail_product')}}",
                data: {id: data}
            }).done((data) => {
        ...
        function deleteMe(id, prd_id) {
        	...
        	$.ajax({
        		url: "{{ route('openitem.openitem_has_qty') }}",
        		type: "POST",
        		data: {
        			product_id: prd_id,
        		},
