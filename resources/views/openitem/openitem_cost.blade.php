...
<div id="landing-view" class="container-fluid">
    <div class="">
		...
		<div class="d-flex mt-0 p-0 row"
			style="width:100%;height:70px;margin-top:5px !important;margin-bottom:5px !important" >
			<div class="col-md-5 align-self-center" style="">
				<h2 style="margin-bottom: 0;">Open Item: Cost</h2>
			</div>

            <div class="col-md-1 align-self-center">
				@if (!empty($prd_info->thumbnail_1))
					<img src="/images/product/{{$prd_info->systemid}}/thumb/{{$prd_info->thumbnail_1}}"
			...
			<div class="col-md-4" style="align-self:center;float:left;padding-left:0">
				<h4 style="margin-bottom:0px;padding-top: 0;line-height:1.5;">
					@if($prd_info->name??""){{$prd_info->name??""}}
					@else Product Name @endif
				</h4>
				<p style="font-size:18px;margin-bottom:0">{{$prd_info->systemid??""}}</p>
...
$('#update_cost_modal').on('hidden.bs.modal', function (e) {
    ...
    $.ajax({
        method: "post",
        url: "{{route('openitem.update_cost')}}",
        data: {
...
var tableData = {
    systemid: "{{request()->route('systemid')}}"
};
var openitemCostTable = $('#openitemCostTable').DataTable({
    ...
    "ajax": {
        ...
        "url": "{{route('openitem_cost.datatable')}}",
        "type": "POST",
        data: function (d) {
