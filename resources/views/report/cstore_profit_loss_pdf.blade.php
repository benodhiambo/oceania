...
<body>
      ...
      C-Store Profit & Loss
		...
		<tr>
			<td valign="bottom" colspan="5" align="right" >
				...
				@if(!empty($requestValue['ev_start_date']) && !empty($requestValue['ev_end_date']))
					{{ date('dMy',strtotime($requestValue['ev_start_date']))}}  - {{ date('dMy',strtotime($requestValue['ev_end_date'])) }}
				@endif
				</p>
			</td>
		</tr>
    ...
    <table border="0" cellpadding="0" cellspacing="1" class="table mt-2" id="item" style="width:100%">
		...
		$num=0;
        ?>
        @foreach($report_details as $key => $rep)
            ...
            <tr class="table-td">
				...
				<td style="text-align:center;border-style: none">
						{{ number_format(($rep->cost),2) }}
				</td>
				<td style="text-align:center;border-style: none">
				{{number_format($rep->qty) ?? 0}}
			    </td>

				<td style="text-align:right;border-style: none;">
				{{ number_format($rep->profit_loss,2) }}
				<?php $grandTotal+= $rep->profit_loss; ?>
			    </td>
            </tr>
        @endforeach
