...
function clear_storage(pmpno) {
    ...
    let current_pds = JSON.parse(localStorage.getItem('pumpDataState'));
    console.log('Current pumpDataState=',JSON.stringify(current_pds))
    ...
    current_pds['pump'+pmpno] = default_pds
    localStorage.setItem('pumpDataState', JSON.stringify(current_pds));

    let current_auth = JSON.parse(localStorage.getItem('authorizeData'));
    ...
    current_auth['pump'+pmpno] = default_auth
    localStorage.setItem('authorizeData', JSON.stringify(current_auth));
...
function pump_idle_status_additions(oldpmpno){

    let grey_cover = localStorage.getItem('scover_grey');
    if ((grey_cover != '') && (grey_cover == oldpmpno)) {
        pump_delivering_status_additions(grey_cover)
    } else {

		// Add in the onclick attribute

        let open_pmp = localStorage.getItem('scover_pump_no');

        if (open_pmp == oldpmpno) {
            $('#scover-' + oldpmpno).attr('hidden', true);
        } else {
            $('#scover-' + oldpmpno).attr('hidden', false);
        }

        $("img.fuelproductimages-" + oldpmpno).each(function (index) {
            $(this).attr("onclick", $(this).attr("bakclick"));
        });

        $("img.iHover").css( "cursor", "pointer" );
    }
}

function update_ui_on_nozzle_down(pmpno) {
    // remove grey cover
    localStorage.removeItem('scover_grey');

    // remove so that product buttons are covered with blue
    localStorage.removeItem('scover_pump_no');

    // set start cover to grey
    $('#scover-' + pmpno).removeClass('scover-grey');
    $('#scover-' + pmpno).addClass('scover');
    $('#scover-' + pmpno + ' > span').html('New');
    $('#scover-' + pmpno).attr('onClick','remove_start_cover('+pmpno+')')
    $('#scover-' + pmpno).css( "cursor", "pointer" );
...
function pump_delivering_status_additions(oldpmpno){

    $('#scover-' + oldpmpno).removeAttr("onclick");

    // set start cover to grey
    $('#scover-' + oldpmpno).addClass('scover-grey');
    $('#scover-' + oldpmpno).removeClass('scover');
    $('#scover-' + oldpmpno).css( "cursor", "default" );
    $('#scover-'+oldpmpno+' > span').html('');

    $('#scover-' + oldpmpno).attr('hidden', false);
}

//additional function when pump is offline
function pump_offline_status_additions(oldpmpno){
    ...
    if ((grey_cover != '') && (grey_cover == oldpmpno)) {
        ...
        $("img.product-disable-offline").removeClass("cursor-pointer");
        $("img.product-disable-offline").css( "cursor", "default" );
