...
function process_finish()
{
	...
	$.post('{{route('local_cabinet.generate_auth_id')}}').done( (res) => { 
		console.log('auth_id', res);
		pumpData[`pump`+selected_pump].auth_id = res;
...
function update_payment_table_new(pump_no, amount = false) {
...
function set_amount(amount)
{
	...
	log2laravel('info', 'set_amount: AUTH amount='+ amount);
	 ...
	 $.ajax({
		...
		success: function (response) {
			...
			if(if_first!=null){
				...
				log2laravel('info', '*****initFuelProduct: pumpno'+my_pump); 
				update_payment_table_new(my_pump);
			}
...
function store_last_filled(auth_id, filled)	{
	$.post( '{{route('local_cabinet.store_last_filled')}}', {auth_id, filled }).
		done((res) => {
