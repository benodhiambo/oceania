...
//	'123456789012345678901234567890123456789012345'
	...
	@foreach ($receiptproduct as $i)
		...
		'{{str_pad($i->discount_pct."%", 4, " ", STR_PAD_LEFT)}}' + '\x09' +
		'{{str_pad(number_format(($receiptdetails->total/100),2), 7, " ", STR_PAD_LEFT)}}' + '\x0A',
	@endforeach
//	'123456789012345678901234567890123456789012345'
	'---------------------------------------------' + '\x0A',
	'Item Amount                        '+ '{{str_pad(number_format($receiptdetails->item_amount / 100,2), 10, " ", STR_PAD_LEFT)}}' + '\x0A',

	'{{!empty($terminal->taxtype)?strtoupper($terminal->taxtype):"SST"}} {{ $terminal->tax_percent}}{{str_pad("%",20," ",STR_PAD_RIGHT)}}{{str_pad(number_format($receiptdetails->sst / 100, 2), 20, " ", STR_PAD_LEFT)}}' + '\x0A',

//	'123456789012345678901234567890123456789012345'
	'{{str_pad("Rounding",25," ",STR_PAD_RIGHT)}}' + '{{str_pad(number_format($receiptdetails->rounding / 100, 2), 20, " ", STR_PAD_LEFT)}}' + '\x0A',
	'---------------------------------------------' + '\x0A',
	'\x1B' + '\x21' + '\x08',	// emphasized mode on
	'Total                    '+'{{str_pad(number_format(($receiptdetails->item_amount / 100) + ($receiptdetails->sst / 100) + ($receiptdetails->rounding /100) , 2), 20, " ", STR_PAD_LEFT)}}' + '\x0A',
	'\x1B' + '\x21' + '\x00',	// emphasized mode off

//	'123456789012345678901234567890123456789012345'
	'Cash Received            '+@if ($receipt->payment_type == "cash")'{{str_pad(!empty($receiptdetails->cash_received)?number_format(($receiptdetails->cash_received/100),2):"0.00", 20, " ", STR_PAD_LEFT)}}'@else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}'@endif + '\x0A',

	'Credit Card              '+ @if ($receipt->payment_type == "creditcard")'{{str_pad(!empty($receiptdetails->creditcard)?number_format(($receiptdetails->creditcard/100),2):"0.00", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',

	'Wallet                   '+ @if ($receipt->payment_type == "wallet")'{{str_pad(!empty($receiptdetails->wallet)?number_format(($receiptdetails->wallet/100),2):"0.00", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',

	'Credit Account              '+ @if ($receipt->payment_type == "creditac")'{{str_pad(!empty($receiptdetails->creditac)?number_format(($receiptdetails->creditac/100),2):"0.00", 20, " ", STR_PAD_LEFT)}}' @else'{{str_pad("0.00", 20, " ", STR_PAD_LEFT)}}' @endif + '\x0A',
	'---------------------------------------------' + '\x0A',
	...
	'{{empty($receipt->currency) ? "MYR": $receipt->currency }} {{(number_format($refund->refund_amount/100,2))}}'+'\x0A',
    '---------------------------------------------' + '\x0A',
	'Item Amount              '+'{{str_pad(number_format($refund->newsales_item_amount/100, 2), 20, " ", STR_PAD_LEFT)}}' + '\x0A',
	'{{"SST " . $terminal->tax_percent}}                 '+'{{str_pad(number_format($refund->newsales_tax/100, 2), 23, " ", STR_PAD_LEFT)}}' + '\x0A',
	'Rounding                 '+'{{str_pad(number_format($refund->newsales_rounding/100, 2), 20, " ", STR_PAD_LEFT)}}' + '\x0A',
