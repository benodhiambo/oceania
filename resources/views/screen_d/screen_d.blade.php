		...
		<div class="col-md-8 hidden-md-down"
			...
			<div>
				...
				<button class="btn btn-success btn-sq-lg screend-button
					bg-screend-purple"
					onclick="window.open('{{route('settin.index')}}','_blank')"
				...
				<button class="btn btn-success btn-sq-lg screend-button
					bg-analytics"
					onclick="window.open('{{route('sales.report')}}','_blank')"
					style="font-size: 14px">
					<span style="">
						Sales Report
					</span>
				...
				<input type="hidden" name="" id="comm_cabinet"
					   value="{{route('comm_cabinet.commCabinet')}}">
				<input type="hidden" name="" id="creditac"
					   value="{{route('creditaccount.get')}}">
				<input type="hidden" name="" id="carpark"
					   value="{{route('carpark.car_park')}}">
			...
			<div>
				...
				<button class="btn btn-success
					...
					onclick="window.open('{{route('screen.e')}}')"
			...
			<div>
				...
				<button class="btn btn-success btn-sq-lg
					...
					onclick="window.open('{{route('local.fuelprice')}}','_blank')"
					style="font-size: 14px">Local Fuel Price
				</button>

				<button class="btn btn-success btn-sq-lg
					screend-button-lg bg-screend-night-black"
					onclick="window.open('{{route('fuel.movement')}}','_blank')"
					style="font-size: 14px">Fuel Movement
				</button>

				<button class="btn btn-success btn-sq-lg
					screend-button-lg bg-screend-night-black"
					onclick="window.open('{{route('local.hydrogenprice')}}','_blank')"
					style="font-size: 14px">Local Hydrogen<br>Price
				</button>

				<button class="btn btn-success btn-sq-lg screend-button
					bg-screend-night-black"
					onclick="window.open('{{route('vehicleMgmt')}}','_blank')"
					style="font-size: 14px">Vehicle
				</button>

				<button class="btn btn-success btn-sq-lg
					bg-screend-night-black screend-button p-0"
					onclick="goTo('creditac')"
					style="font-size: 14px">Credit<br>A/C
				</button>
                <button class="btn btn-success btn-sq-lg
					bg-screend-night-black screend-button p-0"
					onclick="window.open('{{ route('manual_record_list')}}')"
					style="font-size: 14px">Manual Record
				</button>
			...
			<div>
				...
				<button class="btn btn-success btn-sq-lg
					...
					onclick="window.open('{{route('local_price.landing')}}')"
					style="font-size: 14px">Location Price
				</button>

				<button class="btn btn-success btn-sq-lg
					screend-button bg-screend-product"
					onclick="window.open('{{route('openitem.openitem')}}','_blank')"
				...
				<button class="btn btn-success btn-sq-lg screend-button
					...
					bg-screend-product"
					onclick="window.open('{{route('report')}}','_blank')"
					style="font-size: 14px">
					<span style="">
						C-Store<br>Report
					</span>
				...
				<button class="btn btn-success btn-sq-lg
					screend-button-lg bg-screend-product"
					onclick="window.open('{{route('cstore_receiving_notes_list')}}','_blank')"
					style="font-size: 14px">Receiving Note
				</button>

				<button class="btn btn-success btn-sq-lg screend-button
					bg-screend-product"
					onclick="window.open('{{route('cstore_returning_list')}}','_blank')"
					style="font-size: 14px">Returning
				</button>

				<button class="btn btn-success btn-sq-lg
					screend-button-lg bg-screend-product"
					onclick="window.open('{{route('cstore_audited_notes_list')}}','_blank')"
					style="font-size: 14px">Audited Report
				</button>

				<!--
				<button class="btn btn-success screend-button"
					onclick="window.open('{{route('export_stock_ledger_excel')}}')"
					style="font-size: 14px">Stock<br>Ledger
				</button>
				-->
			...
			<div>
				<label>Payment Management</label><br>
				<button class="btn btn-success btn-sq-lg
					screend-button-lg bg-screend-opt-mgmt"
					onclick="window.open('{{route('outdoor.e.wallet.provider.information')}}','_blank')"
					style="font-size: 14px">Outdoor e-Wallet
				</button>
			</div>
			...
			<div class="pl-3"
				...
				<div>
					<div class="mt-2 mb-2 align-items-center">
						<h3><b>OPOSsum</b>
							<span style="font-size:10px">v2.111</span>
				...
				<span class="float-right">
					{{date('dMy H:i:s', strtotime($user->last_login??''))}}
				</span><br>
				<span>Merchant</span>
				...
				<span class="float-right" id="operation_hours">
					{{$location->start_work??''}} - {{$location->close_work??''}}
				</span><br>
				<hr>
				{{--
...
<div style="position: absolute; bottom: 30px; right:30px">
	<span style="position:relative;top:-5px;left:70px;font-size:15px;color:#808080">Ecosystem Powered By
	</span>
	<br>
	<img style="object-fit:contain;width:220px !important"
		 src="{{ asset('images/ocosystem_logo_slgwht-14.png') }}"/>
</div>
					...
					<div class="upload-area" id="uploadfile" style='border:
						...
						@if (!empty($location->e_right_panel_image_file))
							<div id="thumbnail_1" class="thmb" style="">
								...
								@if (in_array(pathinfo($location->e_right_panel_image_file )['extension'], ['mp4', '3gp', 'avi', 'flv', 'mpeg', 'webm', 'ogv']))
									<a href='/images/location/{{$location->id}}/{{$location->e_right_panel_image_file}}'
									   target="_blank">
										<video style="background-color:white;object-fit:cover"
										   width="100%" height="255px" muted loop autoplay>
				...
				<div class="col-md-6">
					<div class="form-group">
						<input type="color" name="e_table_header_color"
						   ...
						   autocomplete="off"
						   value="{{ $location->e_table_header_color }}" required/>
					</div>
					<div class="form-group">
						<input type="color" name="e_right_panel_color"
						   ...
						   autocomplete="off"
						   value="{{ $location->e_right_panel_color }}" required/>
					</div>
					<div class="form-group">
						<input type="color" name="e_bottom_panel_color"
						   ...
						   autocomplete="off"
						   value="{{ $location->e_bottom_panel_color }}" required/>
					</div>
				</div>
...
<script>
	...
	function Setcurrency(currency_id) {
		...
		$.ajax({
			...
			type: 'post',
			data: {
				"_token": "{{ csrf_token() }}", "currency_id": currency_id
			},
			success: function (response) {
	...
	$('#rateModal').on('hidden.bs.modal', function (e) {
		...
		if (data.ratetype || data.textype != old_textype && data.textype || old_sst_gst_vat_field != sst_gst_vat_field) {
			...
			$.ajax({
				...
				type: 'post',
				data: {
					"_token": "{{ csrf_token() }}", "taxtype": data.textype,
	...
	function eod_summarymodel() {
		$.ajax({
			url: "{{route('local_cabinet.eodsummary.popup')}}",
			// headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
			type: 'get',
			success: function (response) {
				// console.log(response);
	...
	async function uploadData(formdata) {
		await del_picture()
		$.ajax({
			url: "{{route('location.saveLocationImage')}}",
			type: 'post',
			data: formdata,
			contentType: false,
			processData: false,
	...
	async function del_picture(id) {
		await $.ajax({
			url: "{{route('location.deleteLocationImage')}}",
			type: 'post',
			data: {
				"location_id": id
			},
	...
	function addThumbnail(data) {
		...
		var video_format = ['mp4', '3gp', 'avi', 'flv', 'mpeg', 'webm', 'ogv', 'mpv'];
		...
		if (video_format.includes(image_ft)) {
			$("#thumbnail_" + num).append('<a target="_blank" style="color:#fff" href="' +
				src + '"><video style="background-color:white;object-fit:contain" width="100%" height="255px" muted loop autoplay> <source src="' + src + '" /></video></a>');
