    ...
    td = $(this).children('td')
      td.each(function(){
        ...
        if (osmanli_calendar.MIN_DATE != 'OFF') {
            if (
				osmanli_calendar.MIN_DATE.getUTCMonth() == osmanli_calendar.CURRENT_DATE.getUTCMonth()
				&&
				osmanli_calendar.MIN_DATE.getUTCFullYear() == osmanli_calendar.CURRENT_DATE.getUTCFullYear()
				) {
        ...
        if (osmanli_calendar.MAX_DATE != "OFF") {
            if (
				osmanli_calendar.MAX_DATE.getUTCMonth() == osmanli_calendar.CURRENT_DATE.getUTCMonth()
				&&
				osmanli_calendar.MAX_DATE.getUTCFullYear() == osmanli_calendar.CURRENT_DATE.getUTCFullYear()
				) {
...
navigationHandler: function(dir) {
    ...
    if ((this.MIN_DATE != 'OFF' && this.disable_back(dir) && dir < 0) || (this.MAX_DATE != 'OFF' && this.disable_back(dir) && dir > 0)) {
        return null;
    }
