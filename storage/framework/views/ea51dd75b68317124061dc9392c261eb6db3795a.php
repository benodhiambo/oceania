        ...
        <div class="modal-content modal-inside bg-purplelobster"
            ...
            <div class="modal-body text-center" style="padding-top: 20px;">
				...
				margin-left: 0.9%;overflow: hidden;width: 100%;'>
                                ...
                                <?php if(!empty($product_details->photo_1)): ?>
                                    <div id="thumbnail_1" class="thmb" style="">
                                        <a href='/<?php echo e(\App\Http\Controllers\OpenitemController::$IMG_PRODUCT_LINK); ?><?php echo e($product_details->systemid); ?>/<?php echo e($product_details->photo_1); ?>'
                                           target="_blank"> <img style="background-color:white;object-fit:contain"
                                                                 src="/<?php echo e(\App\Http\Controllers\OpenitemController::$IMG_PRODUCT_LINK); ?><?php echo e($product_details->systemid); ?>/<?php echo e($product_details->photo_1); ?>"
					...
					border: none; "
                                       onclick='del_picture("<?php echo e($product_details->systemid); ?>");return null'>
                                        <img width="25" src="<?php echo e(asset('images/redcrab_50x50.png')); ?>" alt="">
                                    </a>
					...
					font-size: 17px" id="uploadLogo"
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <input type="text" name="product_name" id="P_name"
                                       ...
                                       autocomplete="off"
                                       value="<?php echo e($product_details->name); ?>" required/>
                            </div>
					   ...
					   <input type="hidden" name="systemid"
                           id="systemid" value="<?php echo e($product_details->systemid); ?>">
                           <input type="hidden" name="product_id"
                           id="product_id" value="<?php echo e($product_details->id); ?>">
                </form>
            </div>
        </div>
    </div>
...
//		console.log(form)
        ...
        $.ajax({
            url: "<?php echo e(route('openitem.updatecustom')); ?>",
            type: "POST",
            enctype: 'multipart/form-data',
...
<script type="text/javascript">
    ...
    <?php if(empty($product_details->prdcategory_id)): ?>
    disable_all();
    ...
    function enable_products() {
        ...
        get_dropDown('#selectproduct', 'product');
            ...
            <?php if(!empty($matrix_ids)): ?>
    var marone_ids = [<?php echo e(implode(',',$matrix_ids)); ?>]
            <?php else: ?>
    var marone_ids = [];
    ...
    function get_dropDown(target, option) {
        ...
        const url = "<?php echo e(route('openitem.get_dropDown',['OPTION'=>'OPTIONV','KEY'=>'KEYV'])); ?>".replace('OPTIONV', option).replace('KEYV', key);
        console.log("url", url);
    ...
    async function del_picture(systemid) {
        await $.ajax({
            url: "<?php echo e(route('openitem.delPicture')); ?>",
            type: 'post',
            data: {
                "systemid": systemid
            },
    ...
    async function uploadData(formdata) {
        await del_picture()
        $.ajax({
            url: '<?php echo e(route('openitem.savePicture')); ?>',
            type: 'post',
            data: formdata,
            contentType: false,
            processData: false,
    ...
    function addThumbnail(data) {
        ...
        $("#uploadfile").append("<button class='redCrabShell' style='position: absolute;bottom: 20px;right: 63px;padding-bottom: 24px;padding-left: 5px;border: none;'  onclick='del_picture(\"<?php echo e($product_details->systemid); ?>\")'><i class='fa fa-times redCrab'></i></button>")
        $("#logo_upload_cam").addClass('green');
    }
</script>
<?php /**PATH /home/user/oceania/trunk/oceania/resources/views/openitem/product_details.blade.php ENDPATH**/ ?>