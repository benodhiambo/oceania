	...
	<div class="d-flex mt-0 p-0"
		...
		<div class="col-sm-3 pl-0">
			<div class="row mb-0 align-items-center"
				...
				<button onclick="window.open('<?php echo e(route("franchise.location_price.stock_in")); ?>')"
				...
				<button onclick="window.open('<?php echo e(route("franchise.location_price.stock_out")); ?>')"
		...
		<tr>
			...
			<th style="width:80px;margin:auto;text-align:center;padding:2px;">
				<button type="button" onclick="select_all_btn(this)"
					...
					<?php echo e($is_all_active == true ? 'active_button_activated':''); ?>"
					data-state="<?php echo e($is_all_active); ?>"
...
<script>
	...
	var franchiseTable = $('#tableFLocaltionProduct').DataTable({
		...
		"autoWidth": false,
		"ajax": {
			"url": "<?php echo e(route('franchise.location_price.datatable')); ?>",
			"type": "POST",
			data: function (d) {
	...
	updateFieldAjax = function (key, val, f_id) {
		$.post('<?php echo e(route('franchise.location_price.price_update')); ?>', {
			"field": key,
			"data": val,
			"product_id": f_id
		}).done(function (res) {
	...
	select_all_btn = function (e) {
		...
		$.post('<?php echo e(route('franchise.location_price.activate_all')); ?>', tableData).done(function (res) {
			franchiseTable.ajax.reload();
		});
	...
	function add_cost_modal(data, old_value , pro, prd_id) {
	  ...
	  $('#add_cost_modal').on('hide.bs.modal', function (e) {
			...
			if (cost_amount !== '') {
				$.ajax({
					url: "<?php echo e(route('local_price.save_prd_cost')); ?>",
					type: "POST",
					data: {
...
<?php echo $__env->make('common.web', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/user/oceania/trunk/oceania/resources/views/local_price/landing_screend.blade.php ENDPATH**/ ?>