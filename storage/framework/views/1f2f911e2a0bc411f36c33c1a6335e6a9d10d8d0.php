	...
	<div class="d-flex" style="width: 100%; height: 75px;">
		...
		<span
			...
			width:150px; text-align: center;"
				id="hrefBtn" href_id="">
			<h5><?php echo e(\App\Models\Location::getLocation()["name"]); ?></h5>
		</span>
		</div>
...
<input type="hidden" name="" id="ipaddr" value="<?php echo e(env('PTS_IPADDR')); ?>">
<?php $__env->stopSection(); ?>
...
<script>
	...
	var table = $('#tankMgt').DataTable({
		...
		"ajax": {
			...
			"url": "<?php echo e(route('tank.list')); ?>",
			"type": "POST",
			data: function (d) {
			...
			{data: 'systemid', name: 'systemid'},
			{
				data: 'product', name: 'product', render: function (data) {
					...
					return data["product"] != null ? data["product"]["thumbnail_1"] != null ?
						"<a href='javascript:void(0)' style='text-decoration: none;padding-top: 15px;' onclick='productResponse(" + data['id'] + ")'> <img width='25px' height='25px' style='margin: 0px 0px 0px 0px' src='/<?php echo e(\App\Http\Controllers\OpenitemController::$IMG_PRODUCT_LINK); ?>" + data["product"]["systemid"] + "/thumb/" + data["product"]["thumbnail_1"] + "' alt=''> " + (getProductionName(data)) + "</a>"
	...
	function save() {
		$.ajax({
			method: "post",
			url: "<?php echo e(route('tank.save')); ?>",
	...
	$('#updateModal').on('hidden.bs.modal', function (e) {
		...
		if (value!=null){
			$.ajax({
				method: "post",
				url: "<?php echo e(route('tank.update_key')); ?>",
	...
	function productResponse(tank) {
		$.ajax({
			method: "post",
			url: "<?php echo e(route('tank.products')); ?>",
			data:{tank:tank}
		}).done((data) => {
			console.log("data",data);
	...
	function yesDelete(){
		$.ajax({
			method: "post",
			url: "<?php echo e(route('tank.delete')); ?>",
			data: {id: $("#prdId").val()}
		})
		.done((data) => {
			console.log("data", data);
...
<?php echo $__env->make('common.web', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/user/oceania/trunk/oceania/resources/views/tank/tankmgmt.blade.php ENDPATH**/ ?>