...
<div class="row"
   ...
   <div class="col-sm-4" style="align-self:center;float:left;padding-left:0">
       <h4 style="margin-bottom:0px;padding-top: 0;line-height:1.5;"><?php if($product->name??""): ?><?php echo e($product->name??""); ?> <?php else: ?> Product Name <?php endif; ?></h4>
...
<script type="text/javascript">
    ...
    var table =$('#ogProductLeger').DataTable({
        ...
        "autoWidth": false,
        "ajax": {
            "url": "<?php echo e(route("fuel_movement.fuel-mouv-receipts-table",[$id,$date])); ?>",
            "type": "get",
            data: function (d) {
...
function showReceipt(data, is_ft, is_oew){
    var url = "<?php echo e(route('fuel.envReceipt')); ?>";
    if(is_ft){
        url ="<?php echo e(route('fulltank.envReceipt')); ?>"
    } else if (is_oew) {
    		url ="<?php echo e(route('oew.get_receiptlist')); ?>"
    }
    ...
    $.ajax({
		...
		function refundMe(id, fuel, filled) {
			$.ajax({
					method: "post",
					url: "<?php echo e(route('oew.refund')); ?>",
					data: {
						id: id,
						filled: filled,
						fuel: fuel
...
<?php echo $__env->make('common.web', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/user/oceania/trunk/oceania/resources/views/fuel_movement/productledger_sale.blade.php ENDPATH**/ ?>