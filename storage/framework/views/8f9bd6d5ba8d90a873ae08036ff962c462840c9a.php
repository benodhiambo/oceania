...
for (i = 1; i <= <?php echo e(env('H2_MAX_PUMPS')); ?>; i++) {
    pumpData['pump' + i] = {
        status: 'Offline',
        volume: "0.00",
        amount: '0.00',
...
log2laravel('***** H2: getTerminalSyncData()***** ');
	...
	$.post('<?php echo e(route('h2_get_sync_data')); ?>').done( (res) => {
...
function h2TerminalSyncData(pump_no) {
  ...
  */
  $.ajax({
    url: "<?php echo e(route('sync-data-h2')); ?>",
    type: 'post',
    headers: {
...
function updatePumpStatusH2(packet, my_pump) {
    ...
    var deleteTerminalSyncData = debounce(function(pump_no) {
        ...
        $.post('<?php echo e(route('h2_delete_sync_data')); ?>', {
            pump_no: pump_no
        }).
        fail((e) => {
...
function process_finish(){
    ...
    $.ajax({
        url: "<?php echo e(url('h2-receipt-list-store')); ?>",
        type: "POST",
        data:data,
        'headers': {
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
function select_custom_preset(){
	...
	if (!isNaN(price)) {
 	...
 	var item_amount = parseFloat(myR / (1 + (<?php echo e($terminal->tax_percent); ?>/100))).toFixed(2);
	if (item_amount == null) {
...
function select_pump(selected_pump) {
    ...
    $.ajax({
        url: "<?php echo e(route('h2-pump-info')); ?>",
        type: "POST",
        data: {
...
<?php /**PATH /home/user/oceania/trunk/oceania/resources/views/h2/h2_product_logic.blade.php ENDPATH**/ ?>