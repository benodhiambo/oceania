<script>
	...
	<?php for($i = 1; $i <= env('MAX_PUMPS'); $i++): ?>
		ft_filter_price("#input-cash-fulltank-<?php echo e($i); ?>","#buffer-input-cash-ft<?php echo e($i); ?>");
		$("#input-cash-fulltank-<?php echo e($i); ?>").on("keyup", (e) => {
	        fulltank_payment['pump'+<?php echo e($i); ?>].dis_cash =
			$('#buffer-input-cash-ft' + <?php echo e($i); ?>).val() != '' ? $('#buffer-input-cash-ft' + <?php echo e($i); ?>).val():0;
			ft_calculate_change(<?php echo e($i); ?>);
	        fulltank_enable_enter(<?php echo e($i); ?>);
		});
	<?php endfor; ?>
	...
	function fulltankGetTerminalSyncData(){
		$.post('<?php echo e(route('ft_get_sync_data')); ?>').done( (res) => {
			if (!res){
			for(i = 1; i <= <?php echo e(env('MAX_PUMPS')); ?>; i++) {
	...
	function process_receipt_ft(sp){
		...
		console.log("MYR ", myr);
		var it_amt = parseFloat((myr / (1 + (<?php echo e($terminal->tax_percent); ?> / 100)))).toFixed(2);
		...
		let _this = this;
		$.ajax({
			url: "<?php echo e(route('ft-create-fuel-list')); ?>",
			type: 'post',
			headers: {
	...
	function fulltankTerminalSyncData(pump_no) {
		...
		*/
		$.ajax({
			url: "<?php echo e(route('sync-data-ft')); ?>",
			type: 'post',
			headers: {
				'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
			},
		...
		/*$.post('<?php echo e(route('sync-data-ft')); ?>', data).
	...
	/* var fulltankDeleteTerminalSyncData = debounce(function(pump_no) {
        ...
        $.post('<?php echo e(route('ft_delete_sync_data')); ?>', {
            pump_no: pump_no
        }).
        fail((e) => console.log(e));
    }, 300);*/
...
<?php /**PATH /home/user/oceania/trunk/oceania/resources/views/fuel_fulltank/fuel_fulltank.blade.php ENDPATH**/ ?>