...
function show_dialog15(e){
	...
	let min_date = '<?php echo e($first_approved); ?>' * 1000
    date = new Date();
...
function open_add_merchant_modal(id) {
	...
	$('#add_merchant_modal').on('hide.bs.modal', function (e) {
		...
		if (merchant_name !== '') {
			$.ajax({
				url: "<?php echo e(route('creditact.save_merchant')); ?>",
				type: "POST",
				data: {
...
function credit_limit_modal(old_value, company) {
	...
	$('#credit_limit_modal').on('hidden.bs.modal', function (e) {
		...
		if (credit_limit != old_value) {
			$.ajax({
				url: "<?php echo e(route('creditact.save_credit_limit')); ?>",
				type: "POST",
				data: {
...
function show_delete_merchant_modal(tag_id){
    ...
    let company_type = $('#'+tag_id).data('ctype');

	$.ajax({
		url: "<?php echo e(route('creditact.check_merchant_account')); ?>",
		type: "POST",
		data: {
			merchant: merchant,
		...
		success: function(response) {
			if (response.account_exists) {
                ...
                $("#delete_company_type").data("ctype", company_type)
...
function delete_company(){
    ...
    let ctype = $('#delete_company_type').data('company_type')

	$.ajax({
		url: "<?php echo e(route('creditact.delete_merchant')); ?>",
		type: "POST",
		data: {
			merchant: merch,
            ctype: ctype
...
function download_ca_stmt(tag_id) {
	...
	$.ajax({
		url: "<?php echo e(route('creditact.download_ca_pdf_stmt')); ?>",
		type: "POST",
		data: {
			'start_date': $('#stmt_start_date').val(),
			'end_date': $('#stmt_end_date').val(),
...
var table = $('#creditact_table').DataTable({
	...
	name: 'amount',
	render: function (data, type, row, meta) {
		var url = '<?php echo e(route("creditacountledger.get", ":id")); ?>';
		...
		render: function (data, type, row, meta) {
			...
			if (row['type'] == 'oneway') {
				company = row['name_company'].replace(/ /g,"_");
                let oneway_sysid = row['sysid']
				return "<a  href='javascript:void(0)' id='del_" + company +
						"' data-is_merchant='true' data-sysid='" + oneway_sysid +"' data-ctype='oneway' style='" + style +
				...
				return "<a  href='javascript:void(0)' id='" + company +
					"' data-is_merchant='false' data  data-ctype='twoway' style='" + style +
					"' onclick='show_delete_merchant_modal(this.id)' class='delete text-right'> "+
					"<img width='25px' src='images/redcrab_50x50.png' alt=''> </a>"
			}
		}
...
<?php echo $__env->make('common.web', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/dev/oceania/trunk/oceania/resources/views/credit_ac/credit_ac.blade.php ENDPATH**/ ?>