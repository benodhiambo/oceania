...
<script>
	...
	*/
    ...
    function process_finish(mypump) {
        ...
        log2laravel('info', 'process_finish: pumpData_1='+JSON.stringify(pumpData['pump' + selected_pump]));
        ...
        log2laravel('info', 'process_finish: pumpData_2='+JSON.stringify(pumpData['pump' + selected_pump]));
        ...
        log2laravel('info', 'process_finish: pumpData_2a='+JSON.stringify(pumpData['pump' + selected_pump]));
                    ...
                    if (pumpData['pump' + selected_pump].payment_type == "Postpaid") {
                        ...
                        if (dis_cash['pump' + selected_pump].payment_type == "cash") {
                            if (pumpData['pump' + selected_pump].preset_type == 'Litre') {
                                var change_amount = dis_cash_ - parseFloat($("#total_amount-main-" +
                                        selected_pump)
                                    .text()
                                    .replace(
                                        /,/g, ''));
        ...
        log2laravel('info', 'process_finish: pumpData_3='+JSON.stringify(pumpData['pump' + selected_pump]));
        ...
        log2laravel('info', 'process_finish: pumpData_4='+JSON.stringify(pumpData['pump' + selected_pump]));
        ...
        log2laravel('info', 'process_finish: pumpData_5='+JSON.stringify(pumpData['pump' + selected_pump]));
        ...
        log2laravel('info', 'process_finish: pumpData_6='+JSON.stringify(pumpData['pump' + selected_pump]));
                ...
                sst = parseFloat(sst) + parseFloat((sum_of_raw_amount) - ((sum_of_raw_amount) / (1 + (
                    <?php echo e($terminal->tax_percent); ?> / 100))));
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        log2laravel('pump_receipt_data_' + selected_pump +
                        ...
                        */
                        ...
                        // Boosuro: This is where we update Fuel Receipt List
                        // dataTable with Javaascript and DataTable API

                        // Boosuro: This is where we store the record in
                        // fuel_receiptlist table

                        // SKumar: This is where we disable the ppump's product buttons
    ...
    function creditact_update(receipt_id, source) {
        $.ajax({
            url: "<?php echo e(route('creditaccount.receiptCreditActionUpdate')); ?>",
                    ...
                    if ((typeof resp.response != 'undefined') &&
                        ...
                        if ((typeof response.Packets != 'undefined') &&
                            ...
                            if ((packet.Type == 'PumpIdleStatus') &&
								...
								//alert('deliveredPump='+deliveredPump);
								...
								log2laravel('info', selected_pump +
									': getPumpStatus 5.3: ERROR! DID NOT MATCH '+
									'RETURN DUE TO: tx_id='+
									tx_id+' AND LastTransaction='+
									LastTransaction);

                                log2laravel('info', selected_pump +
                                    ': getPumpStatus 5.3: packet=' +
                                    JSON.stringify(packet));
                            }
    ...
    function confirmActiveDeliveringPump(pump_id) {
        for (i = 1; i <= <?php echo e(env('MAX_PUMPS')); ?>; i++) { // this was hard coded. Needs revision later
    ...
    function set_amount(dose, id = 'fifty') {
    	console.log('set_amount(): dose=', dose)
        log2laravel('info', selected_pump +
            ': ***** BG1 set_amount(' + dose + ') *****');
	...
	*/
    ...
    function check_enter() {
        console.log('**check_enter()**')
        if (dis_cash['pump' + selected_pump].payment_type == "cash") {
            console.log('check_enter(): pay_type=cash')
            dis_cash_ = (parseFloat(dis_cash['pump' + selected_pump].dis_cash) / 100).toFixed(2);
            if (pumpData['pump' + selected_pump].payment_type == "Postpaid") {
                var amount_total = pumpData['pump' + selected_pump].amount;
                amount_total = ((5 * Math.round((amount_total * 100) / 5)) / 100).toFixed(2);
                ...
                if (dis_cash_ >= rev_cash) {
                    console.log('dis_cash_ == ', dis_cash_);
                    console.log('rev_cash __ == ', rev_cash);
                    console.log('selected_pump = ', selected_pump)
                    if (pumpData['pump' + selected_pump].product) {
                        $('.numpad-enter-payment' + selected_pump).
                        removeClass('poa-button-number-payment-enter-disabled');
    ...
    function void_receipt(pump_no) {
        ...
        $.ajax({
            url: "<?php echo e(route('fuel.voidReceipt')); ?>",
            headers: {
    ...
    function getFuelGradeId(ogfuel_id) {
        ...
        <?php if(!empty($fuel_grade_string)): ?>
			...
			log2laravel('info','getFuelGradeId: fuel_grade_string=' +
				<?php echo json_encode($fuel_grade_string); ?>);
    ...
    function selectProduct(pump_no, ogfuel_id, product_id, product, thumb) {
    ...
    <?php for($i = 1; $i <= env('MAX_PUMPS'); $i++): ?>
        ...
        $("#custom_amount_input_<?php echo e($i); ?>").on("keyup", (e) => {
			...
			if (val == '') {
				$("#custom_amount_btn").addClass('custom-preset-disable');
				$("#custom_amount_btn").removeClass('poa-button-preset');
                $("#custom_amount_btn_<?php echo e($i); ?>").addClass('cover-myr-disable');
                $("#custom_amount_btn_<?php echo e($i); ?>").removeClass('cover-myr-enable');
			} else {
				$("#custom_amount_btn").removeClass('custom-preset-disable');
				$("#custom_amount_btn").addClass('poa-button-preset');

                let buffer_check = $('#custom_amount_input_<?php echo e($i); ?>_buffer').val()

                if(buffer_check > 0) {
                    $("#custom_amount_btn_<?php echo e($i); ?>").removeClass('cover-myr-disable');
                    $("#custom_amount_btn_<?php echo e($i); ?>").addClass('cover-myr-enable');
                }
        ...
        $("#custom_litre_input_<?php echo e($i); ?>").on("keyup", (e) => {
			...
			if (val == '') {
				$("#custom_litre_btn").addClass('custom-preset-disable');
				$("#custom_litre_btn").removeClass('poa-button-preset');
                $("#custom_litre_btn_<?php echo e($i); ?>").addClass('cover-myr-disable');
                $("#custom_litre_btn_<?php echo e($i); ?>").removeClass('cover-myr-enable');
			} else {
				$("#custom_litre_btn").removeClass('custom-preset-disable');
				$("#custom_litre_btn").addClass('poa-button-preset');
                $("#custom_litre_btn_<?php echo e($i); ?>").removeClass('cover-myr-disable');
                $("#custom_litre_btn_<?php echo e($i); ?>").addClass('cover-myr-enable');
			}
        });
    <?php endfor; ?>
...
<!-- FUEL PAGE CODE ENDS -->
<?php /**PATH /home/dev/oceania/trunk/oceania/resources/views/landing/opos_fuelpage.blade.php ENDPATH**/ ?>