...
<div class="container-fluid" style="overflow: auto;">
	...
	<div class="d-flex mt-0" style="width: 100%;">
		...
		<div class="col-md-2 pr-5" style="align-self:center;padding:0px;
		...
		<div class="col-sm-3 pl-0">
			<div class="row mb-0" style="float:right;">
				<button onclick="window.open('<?php echo e(route("fuel.guide")); ?>')"
					class="btn btn-success btn-fuel-guide sellerbutton"
					style="padding:0px;margin-right:5px"
					id="guide_btn">Guide
				</button>

				<form action="<?php echo e(route('fuel_movement.pdf')); ?>" method="post" id="PrintPDF">
					<?php echo csrf_field(); ?>
					<input type="hidden" name="fuel_prod_id" id="_fuel_prod_id">
					<input type="hidden" name="startmonth" id="_startmonth">
					<input type="hidden" name="endmonth" id="_endmonth">
					<input type="hidden" name="fuel_prod_name" id="_fuel_prod_name">
					<input type="hidden" name="date" id="_date">
					<input type="hidden" name="location" id="_location">
					<button	id="PrintPDFButton"
						type="button"
						class="btn btn-success bg-download"
						style="height:70px;width:70px;border-radius:10px;
						outline:none;font-size: 14px; margin-right:0;">PDF
					</button>
				</form>
...
$(document).ready(function(){
	$('#PrintPDFButton').on('click',function(){
		...
		var href_fuel_prod_name = $('#selectFuelModal_btn').attr('href_fuel_prod_name');
		var startDate = $('#month_from').text();
		var location = "<?php echo e($location->name??''); ?>";
		
		$('#_fuel_prod_id').val(fuel_prod_id);
		$('#_startmonth').val(startmonth);
		$('#_endmonth').val(endmonth);
		$('#_fuel_prod_name').val(href_fuel_prod_name);
		$('#_date').val(startDate);
...
// 	$.ajax({
// 			url: "<?php echo e(route('fuel_movement.pdf')); ?>",
// 			async: false,
//             type: "post",
//             'headers': {
...
// 			"endmonth": endmonth,
// 			"fuel_prod_name": href_fuel_prod_name,
// 			"date": startDate
// 		},
// 		success: function(){
		...
		for (var k in response) {
			...
			for (var d = new Date(comp_date_2); d.getMonth() == comp_date_2.getMonth(); d.setDate(d.getDate() + 1)) {
				...
				sales = `<a href="#" style="text-decoration: none;" onclick='openSales("${formateddate}")'>
						${record ? number_format(record.sales,2):'0.00'}</a>`;
...
<?php echo $__env->make('common.web', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/user/oceania/trunk/oceania/resources/views/fuel_movement/fuel_movement.blade.php ENDPATH**/ ?>