...
class invfcc_gatewayd extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'invfcc:gateway';
     ...
     * @var string
     */
    protected $description = 'Invenco FCC Integration Gateway daemon';
		...
		while (true) {
			...
			if (!empty($client)) {
				...
				if (!empty($input) && $input != '\r\n') {
					...
					/*  Input is null empty */
					dump("Input is null empty!");
				}
			} else {
				/*  Client is null empty */
				dump("Client is null empty!");
			}
