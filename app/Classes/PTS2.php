...
class PTS2
{
    ...
    public function __construct($username, $password, $url) {
		...
		/* Have to figure out the user dynamically */
		...
		*/
		...
		switch($client_ip) {
			case env("POS_SERVER_IPADDR"):
				//Log::info('SERVER');
				$this->username = 'server';
				$this->password = 'server';
				break;

			case env("CLIENT_POS1_IPADDR"):
				//Log::info('CLIENT_POS1');
				$this->username = 'clientpos1';
				$this->password = 'clientpos1';
				break;

			case env("CLIENT_POS2_IPADDR"):
				//Log::info('CLIENT_POS2');
				$this->username = 'clientpos2';
				$this->password = 'clientpos2';
				break;

			case env("STATION_MANAGER_IPADDR"):
				//Log::info('STATION MANAGER');
				$this->username = 'stnmgr';
				$this->password = 'stnmgr';
				break;
	...
	*/
	...
	public function pump_authorize($pump_no,$type,$dose,
        ...
        if (!empty($fuelgradeid)) {
            $dataDetails['FuelGradeId'] = (int)$fuelgradeid;
        }
		...
		$this->set_channel();
		//dump($this->ch);
		Log::debug('pump_authorize:'.json_encode($data));
