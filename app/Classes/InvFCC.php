	...
	public function authorize_fuelpoint_response($data, $client, $socket,
		...
		if($pump_status['data']['response']['Packets'][0]['Type'] == 'PumpFillingStatus'){
			...
			if (!empty($xdet['mtype'] &&
				...
				$response = $this->authorize_fuelpoint_raw(
					$data, $restext, $errcode, $errtext, $xdet['trans']);
				var_dump('authorize_fuelpoint_raw: returns out='.$response);

				//$response = json_encode($out);
				//var_dump('authorize_fuelpoint_raw: returns response='.$response);

				$ret0 = $gw->sendEPS($response, $client);
				dump('authorize_fuelpoint_raw: returns from sendEPS: ret0='.$ret0);

				// After Authorization:
				// DeliveryState=New, TransactionSeqNo=XXXX
				$output = $this->delivery_state_change_event(
					$data, $pump_no, "New", $xdet['trans'],
					0, 0, 0, $amount);
				...
				$json1 = $this->fuelpoint_state_change_event(
					$xdet['trans'], $pump_no, 'Authorized',"Valid",
					0, $reservation_id,'NozzleUp');

				$ret2 = $gw->sendEPS($json1, $client);
				dump('fuelpoint_state_change_event: returns from sendEPS: ret2='.$ret2);

				/*
				// Also send DeliveryStarted event to EPS
				$json2 = $this->delivery_started_event(
					$pump_no, $xdet['trans'],
					$prod[$ogfuel_id]->ogfuel_id, 0);

				$ret2 = $gw->sendEPS($json2, $client);
				dump('returns from sendEPS: ret2='.$ret2);
				*/
	...
	public function get_deliverydetails_response($data, $prod) {
		...
		*/
		...
		if (!empty($ptsret)) {
			...
			if(!empty($price) && !empty($volume) && !empty($amount)) {
				$ret = $this->get_deliverydetails_raw($data, $dev_state,
					$pump_no, $transaction_no, $prod[$nozzle]->ogfuel_id,
					$price, $volume, $amount);
			}
		}
