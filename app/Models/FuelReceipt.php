...
class FuelReceipt extends Model
{
    ...
    public static function get_terminal_ids() {
        $ids = [];
        $terminal_ids = DB::table('terminal')->
    ...
    public static function getReceiptValueWithoutVoid($brancheoddata,$eod=false)
    {
        ...
        $fuel_receipts = DB::table('fuel_receipt')->
            selectRaw('fuel_receipt.* , fuel_receiptdetails.* ,
            ...
            whereIn('fuel_receipt.terminal_id', $brancheoddata->terminal_id)->
            whereDate('fuel_receipt.created_at',
            date('Y-m-d', strtotime($brancheoddata->created_at)))->
            whereNotIn('fuel_receipt.status', ['voided', 'refunded'])->
            orderBy('fuel_receipt.id', 'DESC')->get();

        $oew_receipts = DB::table('oew_receipt')->
            selectRaw('oew_receipt.* , oew_receiptdetails.* , oew_receiptlist.* ,
			oew_receipt.id as receipt_id,
			oew_receipt.created_at as created_at ')->
            join('oew_receiptdetails', 'oew_receiptdetails.receipt_id',
            'oew_receipt.id')->
            join('oew_receiptlist', 'oew_receiptlist.oew_receipt_id',
            'oew_receipt.id')->
            whereNull('oew_receipt.deleted_at')->
            whereIn('oew_receipt.terminal_id', $brancheoddata->terminal_id)->
            whereDate('oew_receipt.created_at',
            date('Y-m-d', strtotime($brancheoddata->created_at)))->
            whereNotIn('oew_receipt.status', ['voided', 'refunded'])->
            orderBy('oew_receipt.id', 'DESC')->get();
        ...
        $fulltank_receipts = DB::table('fuelfulltank_receipt')->
            ...
            whereIn('fuelfulltank_receipt.terminal_id', $brancheoddata->terminal_id)->
        ...
        $ev_receipts = DB::table('evreceipt')->
            selectRaw('evreceipt.* , evreceiptdetails.* ,
            evreceipt.id as receipt_id,
            evreceipt.created_at as created_at ')->
            join('evreceiptdetails', 'evreceiptdetails.evreceipt_id',
            'evreceipt.id')->
            whereNull('evreceipt.deleted_at')->
            whereIn('evreceipt.terminal_id', $brancheoddata->terminal_id)->
            whereDate('evreceipt.created_at',
            date('Y-m-d', strtotime($brancheoddata->created_at)))->
            where('evreceipt.status', "!=", 'voided')->
            orderBy('evreceipt.id', 'DESC')->get();

        $cstore_receipts = DB::table('cstore_receipt')->
            selectRaw('cstore_receipt.* , cstore_receiptdetails.*,
			cstore_receipt.id as receipt_id ,
			cstore_receipt.created_at as created_at ')->
            join('cstore_receiptdetails',
            'cstore_receiptdetails.receipt_id', 'cstore_receipt.id')->
            whereNull('cstore_receipt.deleted_at')->
            whereIn('cstore_receipt.terminal_id', $brancheoddata->terminal_id)->
            whereDate('cstore_receipt.created_at',
            date('Y-m-d', strtotime($brancheoddata->created_at)))->
            where('cstore_receipt.status', "!=", 'voided')->
            orderBy('cstore_receipt.id', 'DESC')->get();

        $h2_receipts = DB::table('h2receipt')->
            selectRaw('h2receipt.* , h2receiptdetails.*,
			h2receipt.id as receipt_id,
			h2receipt.created_at as created_at')->
            join('h2receiptdetails',
            'h2receiptdetails.receipt_id', 'h2receipt.id')->
            whereNull('h2receipt.deleted_at')->
            whereIn('h2receipt.terminal_id', $brancheoddata->terminal_id)->
            whereDate('h2receipt.created_at',
            date('Y-m-d', strtotime($brancheoddata->created_at)))->
            where('h2receipt.status', "!=", 'voided')->
            orderBy('h2receipt.id', 'DESC')->get();

        $fuel_refunds = DB::table('fuel_receipt')->
            selectRaw('fuel_receipt.* , fuel_receiptlist.* ,  fuel_receipt.id as receipt_id,
            fuel_receipt.created_at as created_at ')->
            join('fuel_receiptlist', 'fuel_receiptlist.fuel_receipt_id', 'fuel_receipt.id')->
            whereNull('fuel_receipt.deleted_at')->
            whereIn('fuel_receipt.terminal_id', $brancheoddata->terminal_id)->
            whereDate('fuel_receipt.created_at',  date('Y-m-d', strtotime($brancheoddata->created_at)))->
            where('fuel_receipt.status', 'refunded')->
            orderBy('fuel_receipt.id', 'DESC')->
            get();
        Log::info("FR : " . json_encode($fuel_refunds));

        $oew_refunds = DB::table('oew_receipt')->
            selectRaw('oew_receipt.* , oew_receiptlist.* ,  oew_receipt.id as receipt_id,
            oew_receipt.created_at as created_at ')->
            join('oew_receiptlist', 'oew_receiptlist.oew_receipt_id', 'oew_receipt.id')->
            whereNull('oew_receipt.deleted_at')->
            whereIn('oew_receipt.terminal_id', $brancheoddata->terminal_id)->
            whereDate('oew_receipt.created_at',  date('Y-m-d', strtotime($brancheoddata->created_at)))->
            where('oew_receipt.status', 'refunded')->
            orderBy('oew_receipt.id', 'DESC')->
            get();
        ...
        Log::debug('cstore_receipts=' . json_encode($cstore_receipts));
    ...
    public static function getUserLoginReceiptValueWithoutVoid($eod=false)
    {
		...
		Log::info('loginout='.json_encode($loginOut));

        $shift_data = DB::table('pshift')->
            whereId($loginOut->shift_id)->first();

        if (empty($shift_data)) {
            throw new \Exception('Error: Shift Not Found! shift_id='.
				$loginOut->shift_id);
        }
        ...
        $fuel_receipts = DB::table('fuel_receipt')->
            ...
            'fuel_receipt.id')->
            whereNull('fuel_receipt.deleted_at')->
            where('fuel_receipt.created_at', '>=', $loginOut->login)->
            whereIn('fuel_receipt.terminal_id', $shift_data->terminal_id)->
            where('fuel_receipt.staff_user_id', Auth::user()->id)->
            whereNotIn('fuel_receipt.status', ['voided', 'refunded'])->
            orderBy('fuel_receipt.id', 'DESC')->get();

        $oew_receipts = DB::table('oew_receipt')->
            selectRaw('oew_receipt.* , oew_receiptdetails.*,
			oew_receipt.id as receipt_id,
			oew_receipt.created_at as created_at ')->
            join('oew_receiptdetails', 'oew_receiptdetails.receipt_id',
            'oew_receipt.id')->
            whereNull('oew_receipt.deleted_at')->
            where('oew_receipt.created_at', '>=', $loginOut->login)->
            whereIn('oew_receipt.terminal_id', $shift_data->terminal_id)->
            where('oew_receipt.staff_user_id', Auth::user()->id)->
            whereNotIn('oew_receipt.status', ['voided', 'refunded'])->
            orderBy('oew_receipt.id', 'DESC')->get();
        ...
        $fulltank_receipts = DB::table('fuelfulltank_receipt')->
            ...
            whereIn('fuelfulltank_receipt.terminal_id', $shift_data->terminal_id)->
        ...
        $h2_receipts = DB::table('h2receipt')->
            ...
            'h2receiptdetails.receipt_id', 'h2receipt.id')->
            whereNull('h2receipt.deleted_at')->
            where('h2receipt.created_at', '>=', $loginOut->login)->
            whereIn('h2receipt.terminal_id', $shift_data->terminal_id)->
            where('h2receipt.staff_user_id', Auth::user()->id)->
            where('h2receipt.status', "!=", 'voided')->
            orderBy('h2receipt.id', 'DESC')->get();
        ...
        $ev_receipts = DB::table('evreceipt')->
            ...
            'evreceipt.id')->
            whereNull('evreceipt.deleted_at')->
            where('evreceipt.created_at', '>=', $loginOut->login)->
            whereIn('evreceipt.terminal_id', $shift_data->terminal_id)->
            where('evreceipt.staff_user_id', Auth::user()->id)->
            where('evreceipt.status', "!=", 'voided')->
            orderBy('evreceipt.id', 'DESC')->get();
        ...
        $cstore_receipts = DB::table('cstore_receipt')->
			...
			cstore_receipt.id as receipt_id ,
			cstore_receipt.created_at as created_at ')->
            join('cstore_receiptdetails', 'cstore_receiptdetails.receipt_id',
            'cstore_receipt.id')->
            whereNull('cstore_receipt.deleted_at')->
            where('cstore_receipt.created_at', '>=', $loginOut->login)->
            whereIn('cstore_receipt.terminal_id', $shift_data->terminal_id)->
            where('cstore_receipt.staff_user_id', Auth::user()->id)->
            where('cstore_receipt.status', "!=", 'voided')->
            orderBy('cstore_receipt.id', 'DESC')->get();
    ...
    public static function getUserReceiptValueWithoutVoid($loginOut, $eod=false)
    {
        ...
        if ($loginOut->logout == null) {
            $fuel_receipts = DB::table('fuel_receipt')->
                ...
                whereNull('fuel_receipt.deleted_at')->
                where('fuel_receipt.created_at', '>=', $loginOut->login)->
                where('fuel_receipt.staff_user_id', $loginOut->user_id)->
                whereIn('fuel_receipt.terminal_id', $shift_data->terminal_id)->
            ...
            $oew_receipts = DB::table('oew_receipt')->
                ...
                whereNull('oew_receipt.deleted_at')->
                where('oew_receipt.created_at', '>=', $loginOut->login)->
                where('oew_receipt.staff_user_id', $loginOut->user_id)->
                whereIn('oew_receipt.terminal_id', $shift_data->terminal_id)->
            ...
            $cstore_receipts = DB::table('cstore_receipt')->
                ...
                whereNull('cstore_receipt.deleted_at')->
                where('cstore_receipt.created_at', '>=', $loginOut->login)->
                where('cstore_receipt.staff_user_id', $loginOut->user_id)->
                whereIn('cstore_receipt.terminal_id', $shift_data->terminal_id)->
                where('cstore_receipt.status', "!=", 'voided')->
                orderBy('cstore_receipt.id', 'DESC')->get();
            ...
            $fuel_receipts = DB::table('fuel_receipt')->
                ...
                whereNull('fuel_receipt.deleted_at')->
                where('fuel_receipt.created_at', '>=', $loginOut->login)->
                where('fuel_receipt.created_at', '<', $loginOut->logout)->
                whereIn('fuel_receipt.terminal_id', $shift_data->terminal_id)->
                where('fuel_receipt.staff_user_id', $loginOut->user_id)->
                where('fuel_receipt.status', "!=", 'voided')->
                orderBy('fuel_receipt.id', 'DESC')->get();
            ...
            $oew_receipts = DB::table('oew_receipt')->
                ...
                whereNull('oew_receipt.deleted_at')->
                where('oew_receipt.created_at', '>=', $loginOut->login)->
                where('oew_receipt.created_at', '<', $loginOut->logout)->
                where('oew_receipt.staff_user_id', $loginOut->user_id)->
                whereIn('oew_receipt.terminal_id', $shift_data->terminal_id)->
                where('oew_receipt.status', "!=", 'voided')->
                orderBy('oew_receipt.id', 'DESC')->get();
            ...
            $cstore_receipts = DB::table('cstore_receipt')->
                ...
                whereNull('cstore_receipt.deleted_at')->
                where('cstore_receipt.created_at', '>=', $loginOut->login)->
                where('cstore_receipt.created_at', '<', $loginOut->logout)->
                whereIn('cstore_receipt.terminal_id', $shift_data->terminal_id)->
                where('cstore_receipt.staff_user_id', $loginOut->user_id)->
                where('cstore_receipt.status', "!=", 'voided')->
                orderBy('cstore_receipt.id', 'DESC')->get();
