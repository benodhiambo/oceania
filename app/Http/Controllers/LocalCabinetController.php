...
class LocalCabinetController extends Controller
{
    ...
    public function update_ps_cstore($terminal_id, $shift_id, $user_id, $from, $to)
    {
        ...
        $total = DB::table('cstore_receiptdetails')->join('cstore_receipt', 'cstore_receiptdetails.receipt_id', '=', 'cstore_receipt.id')->whereNotIn('cstore_receipt.status', ['refunded', 'voided'])->where('cstore_receipt.terminal_id', $terminal_id)->where('cstore_receipt.staff_user_id', '=', $user_id)->whereBetween('cstore_receiptdetails.created_at', [$from, $to])->sum('total');

        // Log::info('update_ps_cstore: total='.json_encode($total));

        $refunded = DB::table('cstore_receiptdetails')->join('cstore_receipt', 'cstore_receiptdetails.receipt_id', '=', 'cstore_receipt.id')->join('cstore_receiptrefund', 'cstore_receiptrefund.cstore_receipt_id', '=', 'cstore_receipt.id')->where('cstore_receipt.status', '=', 'refunded')->where('cstore_receipt.terminal_id', $terminal_id)->where('cstore_receipt.staff_user_id', '=', $user_id)->whereBetween('cstore_receiptdetails.created_at', [$from, $to])->sum('cstore_receiptrefund.refund_amount');
        ...
        if ($total > 0) {
            ...
            $shift_data = DB::table('pshiftdetails')->where('pshift_id', '=', $shift_id)->first();

            // Log::info('update_ps_cstore: shift_data='.
            // json_encode($grand_total));

            if ($shift_data == null) {
                $ret = DB::table('pshiftdetails')->insert([
                    "pshift_id" => $shift_id,
                    "cstore" => $grand_total,
                    "created_at" => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s'),
                ...
                $ret = DB::table('pshiftdetails')->where('pshift_id', '=', $shift_id)->update([
                    "cstore" => $grand_total,
                    'updated_at' => date('Y-m-d H:i:s'),
                ]);
    ...
    public function update_shiftfuel($terminal_id, $shift_id, $user_id, $from, $to)
    {
        ...
        $shift_data = DB::table('pshiftfuel')->where('pshift_id', '=', $shift_id)->first();

        $products = DB::table('prd_ogfuel')->join('product', 'product.id', '=', 'prd_ogfuel.product_id')->where('prd_ogfuel.status', 'active')->select('product.id', 'product.name', 'prd_ogfuel.id as ogfuel_id')->get();
        ...
        foreach ($products as $product) {
            ...
            $total = DB::table('fuel_receiptdetails')->join('fuel_receiptproduct', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receiptdetails.receipt_id')->join('fuel_receipt', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receipt.id')->join('fuel_receiptlist', 'fuel_receiptlist.fuel_receipt_id', '=', 'fuel_receiptdetails.receipt_id')->where('fuel_receiptproduct.product_id', '=', $product->id)->where('fuel_receipt.terminal_id', $terminal_id)->where('fuel_receipt.status', '!=', 'voided')->where('fuel_receiptlist.status', '!=', 'refunded')->where('fuel_receipt.staff_user_id', '=', $user_id)->whereNotIn('fuel_receiptlist.status', ['refunded', 'voided'])->whereBetween('fuel_receiptdetails.created_at', [$from, $to])->sum('fuel_receiptdetails.total');

            // Log::info('update_shiftfuel: total='.json_encode($total));

            $ft_total = DB::table('fuelfulltank_receiptdetails')->join('fuelfulltank_receiptproduct', 'fuelfulltank_receiptproduct.fulltank_receipt_id', '=', 'fuelfulltank_receiptdetails.fulltank_receipt_id')->join('fuelfulltank_receipt', 'fuelfulltank_receiptproduct.fulltank_receipt_id', '=', 'fuelfulltank_receipt.id')->join('fuelfulltank_receiptlist', 'fuelfulltank_receiptlist.fuel_fulltank_receipt_id', '=', 'fuelfulltank_receiptdetails.fulltank_receipt_id')->where('fuelfulltank_receiptproduct.product_id', '=', $product->id)->where('fuelfulltank_receipt.terminal_id', $terminal_id)->where('fuelfulltank_receipt.status', '!=', 'voided')->where('fuelfulltank_receiptlist.status', '!=', 'refunded')->where('fuelfulltank_receipt.staff_user_id', '=', $user_id)->whereNotIn('fuelfulltank_receiptlist.status', ['refunded', 'voided'])->whereBetween('fuelfulltank_receiptdetails.created_at', [$from, $to])->sum('fuelfulltank_receiptdetails.total');

            // Log::info('update_shiftfuel: ft_total='.json_encode($ft_total));

            $rounding = DB::table('fuel_receiptdetails')->join('fuel_receiptproduct', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receiptdetails.receipt_id')->join('fuel_receipt', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receipt.id')->where('fuel_receiptproduct.product_id', '=', $product->id)->where('fuel_receipt.status', '!=', 'voided')->where('fuel_receipt.terminal_id', $terminal_id)->where('fuel_receipt.staff_user_id', '=', $user_id)->whereBetween('fuel_receiptdetails.created_at', [$from, $to])->sum('fuel_receiptdetails.rounding');

            // Log::info('update_shiftfuel: rounding='.json_encode($rounding));

            $ft_rounding = DB::table('fuelfulltank_receiptdetails')->join('fuelfulltank_receiptproduct', 'fuelfulltank_receiptproduct.fulltank_receipt_id', '=', 'fuelfulltank_receiptdetails.fulltank_receipt_id')->join('fuelfulltank_receipt', 'fuelfulltank_receiptproduct.fulltank_receipt_id', '=', 'fuelfulltank_receipt.id')->where('fuelfulltank_receiptproduct.product_id', '=', $product->id)->where('fuelfulltank_receipt.status', '!=', 'voided')->where('fuelfulltank_receipt.terminal_id', $terminal_id)->where('fuelfulltank_receipt.staff_user_id', '=', $user_id)->whereBetween('fuelfulltank_receiptdetails.created_at', [$from, $to])->sum('fuelfulltank_receiptdetails.rounding');

            // Log::info('update_shiftfuel: ft_rounding='.json_encode($ft_rounding));

            $newsales_total = DB::table('fuel_receiptdetails')->join('fuel_receiptproduct', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receiptdetails.receipt_id')->join('fuel_receipt', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receipt.id')->join('fuel_receiptlist', 'fuel_receiptlist.fuel_receipt_id', '=', 'fuel_receiptdetails.receipt_id')->where('fuel_receiptproduct.product_id', '=', $product->id)->where('fuel_receiptlist.status', '=', 'refunded')->where('fuel_receipt.terminal_id', $terminal_id)->where('fuel_receipt.staff_user_id', '=', $user_id)->whereBetween('fuel_receiptlist.created_at', [$from, $to])->sum('fuel_receiptlist.newsales_item_amount');

            // Log::info('update_shiftfuel: newsales_total='.json_encode($newsales_total));

            $newsales_rounding = DB::table('fuel_receiptdetails')->join('fuel_receiptproduct', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receiptdetails.receipt_id')->join('fuel_receipt', 'fuel_receiptproduct.receipt_id', '=', 'fuel_receipt.id')->join('fuel_receiptlist', 'fuel_receiptlist.fuel_receipt_id', '=', 'fuel_receiptdetails.receipt_id')->where('fuel_receiptproduct.product_id', '=', $product->id)->where('fuel_receiptlist.status', '=', 'refunded')->where('fuel_receipt.terminal_id', $terminal_id)->where('fuel_receipt.staff_user_id', '=', $user_id)->whereBetween('fuel_receiptlist.created_at', [$from, $to])->sum('fuel_receiptlist.newsales_rounding');
    ...
    public function localCabinetDataTable()
    {
        ...
        $dataReceipt = DB::table('cstore_receipt')->select('created_at')->orderBy("id", "desc")->get();

        $dataReceipt_3 = DB::table('fuel_receipt')->select('created_at')->orderBy("id", "desc")->get();

        $dataReceipt_4 = DB::table('evreceipt')->select('created_at')->orderBy("id", "desc")->get();
        $dataReceipt_ft = DB::table('fuelfulltank_receipt')->select('created_at')->orderBy("id", "desc")->get();

        $dataReceipt_h2 = DB::table('h2receipt')->select('created_at')->orderBy("id", "desc")->get();

        $dataReceipt_oew = DB::table('oew_receipt')->select('created_at')->orderBy("id", "desc")->get();

        $dataReceipt = $dataReceipt->merge($dataReceipt_3);
        $dataReceipt = $dataReceipt->merge($dataReceipt_4);
        $dataReceipt = $dataReceipt->merge($dataReceipt_ft);
        $dataReceipt = $dataReceipt->merge($dataReceipt_h2);
        $dataReceipt = $dataReceipt->merge($dataReceipt_oew);

        //update eod
        $brancheoddatas = DB::table('brancheod')->get();

        foreach ($brancheoddatas as $brancheoddata) {
        ...
        $dataReceipt = $dataReceipt->groupBy(function ($date) {
            return Carbon::parse($date->created_at)->format('dMY'); // grouping by months
        });

        // Log::info('AFTER dataReceipt groupBy : ' . json_encode($dataReceipt));

        $receipt = collect();

        $dataReceipt->map(function ($z) use ($receipt) {

            Log::info('dataReceipt map count(z)=' . count($z));
            Log::info('Current Date -> ' . $z->min('created_at'));
            ...
            $packet->sortdate = strtotime($z->min('created_at'));

            $packet->shift = DB::table('loginout')->whereDate('created_at', date(
                "Y-m-d",
                strtotime($z->min('created_at'))
            ))->count();
        ...
        return Datatables::of($receipt)
            ->addIndexColumn()
            ->addColumn('eod', function ($receipt) {
                $day_passed = "'" . $receipt->date . "'";
                $eod_url = '<a href="#" onclick="eod_summarylist(' . $day_passed . ')" style="margin:0; text-decoration: none;">' . $receipt->date . '</a>';
                return <<<EOD
					$eod_url
EOD;
            })
...
EOD;
            ...
            ->addColumn('nshift', function ($receipt) {
                $day_passed = $receipt->date;
                $shift_url = '<a href="' . route("local_cabinet.nshift", ['date' => $day_passed]) . '" target="_blank" style="margin:0; text-decoration: none;">Shift</a>';
                return <<<EOD
					$shift_url
EOD;
            })
...
EOD;
    ...
    public function localcabinaet_nshit(Request $request)
    {
        $company = Company::first();
    ...
    public function nshift_datatable()
    {
        $date = request()->date;
        $data = array((object) [
            'in' => '17Jun22 20:46:53',
            'out' => '17Jun22 20:46:53',
            'staff_id' => '1234567890',
            'staff_name' => 'Peter Pan',
        ]);
        return Datatables::of($data)
            ->addIndexColumn()
            ->addIndexColumn('in', function ($dt) {
                return $dt->in;
            })
            ...
            ->addColumn('action', function ($dt) {
                return '<a  href="javascript:void(0)" onClick="openShiftModal('.$dt->staff_id.')"  style=" filter: background: red; /*pointer-events: none; */cursor: pointer;" data-row="0" class="delete"> <img width="25px" src="' . asset("images/bluecrab_50x50.png") . '"" alt=""> </a>';
            })
            ->escapeColumns([])
            ->make(true);
    }
    ...
    public function cstore_receipt_landing(Request $request)
    {
        ...
        )->whereMonth('created_at', '=', date('m', strtotime($request->date)))->whereYear('created_at', '=', date('Y', strtotime($request->date)))->orderBy("id", "desc")->get();

        $date = $request->date;
        $receipt->map(function ($z) {
            $z->is_refunded = !empty(DB::table('cstore_receiptrefund')->where('cstore_receipt_id', $z->id)->first());
        });

        return view(
            'cstore.cstore_receiptlist',
            compact('receipt', 'terminal', 'location', 'date', 'approved_at')
        );
    }
    ...
    public function cstore_table_search_ptype(Request $request)
    {
        ...
        $receipt = DB::table('cstore_receipt')->whereDay(
            ...
            '=',
            date('d', strtotime($date))
        )->where('cstore_receipt.payment_type', 'like', $ptype . '%')->whereMonth('created_at', '=', date('m', strtotime($date)))->whereYear('created_at', '=', date('Y', strtotime($date)))->orderBy("id", "desc")->get();
    ...
    public function cstoreVoidReceipt(Request $request)
    {
        ...
        if ($brancheoddata != null) {
            ...
            $loginOut = DB::table('loginout')->where("user_id", $cstore_receipt->staff_user_id)->where("login", "<=", $cstore_receipt->created_at)->where("logout", ">", $cstore_receipt->created_at)->first();

            if ($loginOut == null) {
                $loginOut = DB::table('loginout')->where("user_id", $cstore_receipt->staff_user_id)->where("login", "<=", $cstore_receipt->created_at)->where("logout", null)->first();
            }
    ...
    public function eodSummaryPopup(Request $request)
    {
        ...
        $refund_data = DB::table('receiptrefund')->join('receipt', 'receipt.id', 'receiptrefund.receipt_id')->whereDate('receipt.created_at', '=', $date_eod)->get()->sum('refund_amount');
    ...
    public function eodReceiptPopup(Request $request)
    {
        ...
        $receiptproduct = DB::table('cstore_receiptproduct')->
			where('cstore_receiptproduct.receipt_id', $receipt->id)->get();
    ...
    public function ReceiptRefund(Request $request)
    {
        ...
        $receiptproduct = DB::table('receiptproduct')->join('prd_ogfuel', 'prd_ogfuel.product_id', 'receiptproduct.product_id')->where('receiptproduct.receipt_id', $request->receipt_id)->select('prd_ogfuel.id as og_id', 'receiptproduct.*')->first();
    ...
    public function ReceiptRefund_cstore(Request $request)
    {
        try {
            ...
            $cstore_receiptrefund = DB::table('cstore_receiptrefund')->where('id', $refund_id)->get();
            $cstore_receipt = DB::table('cstore_receipt')->where('id', $request->receipt_id)->first();
    ...
    public function ReceiptCreateCStore(Request $request)
    {
        ...
        try {
            ...
            foreach ($products as $p) {
                ...
                if ($exisitingQty < $req_qty && $company->auto_stockin) {
                ...
                if ($prd->ptype == 'inventory') {
                    ...
                    app("App\Http\Controllers\CentralStockMgmtController")->
                        process_locprod_stockout($prd->systemid, $p['qty'] * -1, $rid);

                } else if ($prd->ptype == 'openitem') {

                    app("App\Http\Controllers\OpenitemController")->
                        process_openitem_stockout($prd->systemid, $p['qty'] * -1);
