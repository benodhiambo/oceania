...
EOD;
    ...
    public function local_price_datatable(Request $request)
    {
        try {
            $data = DB::table('product')->
                leftjoin('prd_inventory', 'prd_inventory.product_id',
                'product.id')->
...
EOD;
                ...
                if (!empty($memberList->product_id)) {
                    ...
                    /*  Log::debug('local_price_datatable: addColumn lprec:' .
                    json_encode($lprec)); */

                    $prd_cost = empty($lprec->cost) ? 0 : $lprec->cost;

                    /*   Log::debug('local_price_datatable: addColumn prd_cost=:' .
                json_encode($prd_cost));
                 */
                } else {
                    $prd_cost = 0;
                }
                ...
                return <<<EOD
					<a href="local_price/locprod_cost/$memberList->systemid" id="cost_$memberList->systemid" style="text-decoration: none;" target="_blank"
                    >
                        $formatted
					</a>
EOD;
                ...
                $cost_value = DB::select(
                    DB::raw("
                        select sum(lp.cost * lp.balance) as total from locationproduct_cost lp, product p, locprod_productledger lpl where p.systemid = lpl.product_systemid and lpl.id = lp.locprodprodledger_id and p.id = '" . $memberList->product_id . "'
                    ")
                );
...
EOD;
                ...
                if (!empty($data->recommended_price)) {
                    ...
                    $qty = app("App\Http\Controllers\CentralStockMgmtController")->qtyAvailable($data->z_product_id);
...
EOD;
    ...
    public function locationproduct_cost(Request $request)
    {
        ...
        app("App\Http\Controllers\CentralStockMgmtController")->
            reflect_autostock_locationproduct_cost();
    ...
    public function locprod_cost_datatable(Request $request)
    {
        try {
            ...
            $cost_data = DB::table('locprod_productledger')->
                ...
                join('locationproduct_cost', 'locationproduct_cost.locprodprodledger_id', 'locprod_productledger.id')->
                select(
            ...
            return Datatables::of($updated_data)->
                ...
                if (empty($so_qty) || $so_qty == 0) {
                    ...
                    return '<a href="#" onclick="show_doc_qty_modal(' . $data->stockout . ')" style="text-decoration:none;" class="">' . $so_qty . ' </a>';
                }
