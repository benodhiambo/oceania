...
class APIFcController extends Controller
{
	...
	*/
	...
	public function updateCompany($c) {
		try {

			\Log::info(['company' => $c ]);
			
			DB::table('company')->
			where('systemid', $c['systemid'])->
			update([
				'loyalty_pgm' => $c['loyalty_pgm'],
				'updated_at' => now()
			]);


			DB::table('prd_openitem')->
			update([
				'loyalty' => $c['loyalty_pgm'],
				'updated_at' => now()
			]);
	...
	public function insertPrdInventory($rec) {
		try {
			...
			\Log::info("insertPrdInventory data:". json_encode($rec));
	...
	public function insertProductbarcode($bc) {
		try {
			...
			Log::debug('insertProductbarcode: product_rec='.
				json_encode($product_rec));
...
//				'product_id' => $product_rec->id
			...
			Log::debug('insertProductbarcode: bc='.json_encode($bc));
			Log::debug('insertProductbarcode: conditions='.
	...
	public function insertLocationPrice($lp) {
		try {
			...
			if (!empty($is_exist))
				$lp['recommended_price'] = $is_exist->recommended_price;
	...
	private function updateOrInsert($tableName, $targetCondition, $targetData) {
		try {
			...
			Log::debug('updateOrInsert: shouldInsert='.
				json_encode($shouldInsert));
