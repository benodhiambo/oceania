...
class FuelReceiptController extends Controller
{
    ...
    public function fuelReceipt(Request $request)
    {
        ...
        try {
            ...
            $refund = FuelReceiptList::where("fuel_receipt_id", $request->id)->join('users', 'users.id', 'fuel_receiptlist.refund_staff_user_id')->first();

            $receiptCount = DB::table('creditact_ledger')->where('document_no', $request->id)->get()->count();
    ...
    public function fuelRefund(Request $request)
    {
        ...
        $price = DB::table('fuel_receiptproduct')->join('fuel_receipt', 'fuel_receipt.id', 'fuel_receiptproduct.receipt_id')->join('product', 'product.id', 'fuel_receiptproduct.product_id')->where('fuel_receipt.status', '!=', 'voided')
        ...
        DB::table('fuel_receiptlist')->where([
            "fuel_receipt_id" => $request->id,
        ])->update([
            "status" => 'refunded',
            "refund" => ($request->fuel - $request->filled) * 100,
        ...
        /*$rec = DB::table('fuel_receipt')
            ->select('fuel_receipt.*', 'fuel_receiptdetails.*', 'fuel_receiptlist.refund', 'fuel_receiptlist.refund_qty')
            ->join('fuel_receiptdetails', 'fuel_receiptdetails.receipt_id', 'fuel_receipt.id')
        ...
        DB::table('fuel_receiptdetails')->where([
        ...
        DB::table('fuel_receiptproduct')->where([
            "receipt_id" => $request->id,
        ])->update([
            "quantity" => DB::raw('quantity - ' . $rec->refund_qty),
        ]);*/
        $brancheoddata = DB::table('brancheod')->whereDate('created_at', '=', date('Y-m-d', strtotime($rec->created_at)))->first();
        \Illuminate\Support\Facades\Log::info(json_encode($brancheoddata));
    ...
    public function dataTable(Request $request)
    {
        try {
			...
			fuel_receiptlist.created_at AS created_at')->whereNull('fuel_receiptlist.deleted_at')->whereDate('fuel_receiptlist.created_at', date('Y-m-d', strtotime($request->date)))->orderBy('fuel_receiptlist.id', 'DESC')->get();
    ...
    public function datatable_search_pump(Request $request)
    {
        try {
            $data = DB::table('fuel_receipt')->join('fuel_receiptlist', 'fuel_receiptlist.fuel_receipt_id', 'fuel_receipt.id')->selectRaw('fuel_receipt.status,
    			...
    			fuel_receiptlist.pump_no,
    			fuel_receiptlist.fuel_receipt_id AS receipt_id,
    			fuel_receiptlist.created_at AS created_at')->whereNull('fuel_receiptlist.deleted_at')->where('fuel_receiptlist.pump_no', 'like', $request->pump_no . '%')->whereDate('fuel_receiptlist.created_at', date('Y-m-d', strtotime($request->date)))->orderBy('fuel_receiptlist.id', 'DESC')->get();
    ...
    public function datatable_search_ptype(Request $request)
    {
        ...
        try {
            $data = DB::table('fuel_receipt')->join('fuel_receiptlist', 'fuel_receiptlist.fuel_receipt_id', 'fuel_receipt.id')->selectRaw('fuel_receipt.status,
              ...
              fuel_receiptlist.pump_no,
              fuel_receiptlist.fuel_receipt_id AS receipt_id,
              fuel_receiptlist.created_at AS created_at')->whereNull('fuel_receiptlist.deleted_at')->where('fuel_receipt.payment_type', 'like', $request->ptype . '%')->whereDate('fuel_receiptlist.created_at', date('Y-m-d', strtotime($request->date)))->orderBy('fuel_receiptlist.id', 'DESC')->get();
...
EOD;
    ...
    public function voidedReceipt(Request $request)
    {
        try {
            ...
            $brancheoddata = DB::table('brancheod')->whereDate('created_at', '=', date('Y-m-d', strtotime($fuel_receipt->created_at)))->first();
            \Illuminate\Support\Facades\Log::info(json_encode($brancheoddata));
            ...
            */
            ...
            $creditact_ledger = DB::table('creditact_ledger')->where([
            ...
            if (!empty($creditact_ledger)) {
                $creditact = DB::table('creditact')->whereId($creditact_ledger->creditact_id)->first();

                DB::table('creditact_ledger')->where('document_no', $request->receipt_id)->update([
                        "amount" => 0.00
                    ]);

                DB::table('creditact')->where('id', $creditact_ledger->creditact_id)->update([
            ...
            if ($brancheoddata != null) {
                ...
                $loginOut = DB::table('loginout')->where("user_id", $fuel_receipt->staff_user_id)->where("login", "<=", $fuel_receipt->created_at)->where("logout", ">", $fuel_receipt->created_at)->first();

                if ($loginOut == null) {
                    $loginOut = DB::table('loginout')->where("user_id", $fuel_receipt->staff_user_id)->where("login", "<=", $fuel_receipt->created_at)->where("logout", null)->first();
                }
