...
class CreditAccountController extends Controller
{
    function creditAccount()
    {
        try {
            ...
            return view('credit_ac.credit_ac',
                compact('getCompany' , 'getoneway' , 'getmerlink', 'data', 'first_approved'));
    ...
    public function CreditAccountLedgerDatatable(Request $request){
        try {
                ...
                if ($item->status == 'refunded') {
                  $fuel_refunded = DB::table('fuel_receipt')->
                    ...
                    $refunded_amount = $fuel_refunded->total -
						...
						$c_count = DB::table('creditact_ledger')->where('document_no',$item->sysid)->count();
            ...
            Log::debug('creditact_ledger: creaditact='.json_encode($creditact));
