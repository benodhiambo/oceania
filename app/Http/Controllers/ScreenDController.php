...
class ScreenDController extends Controller
{
   /* public function __construct()
    ...
    public function screen_d()
    {
        ...
        return view('screen_d.screen_d',
			compact('terminal', 'location', 'user', 'merchant'));
    }
    ...
    public function screen_oceania_d()
    {
        ...
        return view('oceania_svr.screen_d.screen_d',
			compact('terminal', 'location', 'user', 'merchant'));
    }
    ...
    public function fuel_movement()
    {
        ...
        return view('fuel_movement.fuel_movement',
			compact('location', 'company', 'set_update'));
    }
    ...
    public function settingindex()
    {
        ...
        return view('setting.setting',
			compact('company', 'location', 'companydirector', 'companycontact'));
    }
	...
	public function getproduct(Request $request)
    {
        ...
        $product_type = DB::table('product')
            ->select("product.ptype")
            ->where('product.id', $a)
            ->first();

        if($product_type->ptype=='openitem'){
            ...
            whereNotIn('product.ptype', ['oilgas','openitem'])->
            where('product.id', $a)->
            get();
        }

        Log::info('getproduct: productData='.
	...
	public function screen_a_products($a = null) {
		...
		if (empty($a))
			...
			Log::info('product_data_open_item ='.
				json_encode($product_data_open_item));


			$product_data  = $product_data->
				merge($product_data_open_item)->unique('systemid');
    ...
    public function turn_on_auto_stockin_setting(){
        $company = Company::first();
        $company->auto_stockin = 1;
        $company->save();
        return response()->Json(['status' => 'successfully turned on auto stockin setting']);
    }

    public function turn_off_auto_stockin_setting(){
        $company = Company::first();
        $company->auto_stockin = 0;
        $company->save();
        return response()->Json(['status'=>'successfully turned off']);
    }
